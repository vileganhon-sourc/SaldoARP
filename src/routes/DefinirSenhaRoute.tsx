...
export const DefinirSenhaRoute: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        setErrorMsg(error.message || 'Falha ao definir senha de acesso.');
      ...
      setErrorMsg(err.message || 'Erro inesperado ao definir senha.');
      setIsSubmitting(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} style={{ padding: '1.75rem 2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {success ? (
              <div style={{ padding: '1.5rem', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '6px', color: '#166534', textAlign: 'center' }}>
                ...
                <h3 style={{ fontSize: '1rem', fontWeight: 700, margin: '0 0 0.25rem' }}>Senha definida com sucesso!</h3>
                <p style={{ margin: 0, fontSize: '0.8rem', color: '#15803d' }}>
                  Redirecionando para a tela principal do ComprasSUSP...
                </p>
              </div>
            ) : (
              <>
